...
bool contains(const string& haystack,
              const string& needle)
{
    for (char c: needle)
      if (string::npos == haystack.find(c))
        return false;
    return true;
}

uint32_t digit(
    const array<string,15>& s,
    const string& v)
{
    switch(v.size())
    {
        ...
        case 6:
          if (contains(v, s[2])) return 9;
          if (contains(v, s[1])) return 0;
          return 6;
        
        case 5:
          if (contains(v, s[0])) return 3;
          if (contains(v, s[10])) return 5;
          return 2;
    }
...
const char* input0 = R"INPUT(be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb |
...
fgae cfgab fg bagce)INPUT";

const char* input = R"INPUT(bgeacd dbfag bcadegf agdce dgfbce bgc bdgca aedcgf bc abec | gcdfbe cbea bc gbc
...
gfbaed fcabde gdbfc bdeafgc dgb agfcdb cgda fcdab gfbec gd | becadf facdeb dg agbdcf)INPUT";